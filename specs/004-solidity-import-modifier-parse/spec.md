...
**Input**: Fix (A) duplicate contract tile on Show imports for `tests/fixtures/solidity/ImportModifierFixture.sol`, (B) split/malformed `override(...)` and signature tokens shown as modifier nodes.

## User Scenarios

### US1 - No phantom contract on Show imports (P1)

**Given** single-file analysis of `ImportModifierFixture.sol`, **When** user toggles **Show imports**, **Then** exactly one `ImportModifierFixture` type compound exists and unused file-level import edges originate from `type:ImportModifierFixture`, not a separate `external:ImportModifierFixture` stub.

**Why this priority**: Duplicate contract tiles break import overview and suggest a second unknown contract.

**Independent Test**: Graph payload has no node with `id=external:ImportModifierFixture` and no `group=external` node labeled `ImportModifierFixture`.
...
**Independent Test**: Unit test on representative header; graph for `ImportModifierFixture.sol` has no modifier node whose label contains an unmatched `(`.
...
- **FR-004**: Regression tests MUST cover ImportModifierFixture import graph and override tokenization.
...
- **SC-001**: `tests/fixtures/solidity/ImportModifierFixture.sol` graph has zero `external:ImportModifierFixture` / duplicate external contract nodes.
