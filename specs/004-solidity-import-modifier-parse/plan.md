...
Two root causes in ImportModifierFixture single-file graph:

1. **Phantom ImportModifierFixture**: `_append_import_usage_edges` emits unused imports with `source_name=contract_name`. `resolve_endpoint` fails to find `function:ImportModifierFixture.ImportModifierFixture` and creates `external:ImportModifierFixture`.
...
| `tests/unit/test_solidity_import_modifier_graph.py` | ImportModifierFixture graph regression |
