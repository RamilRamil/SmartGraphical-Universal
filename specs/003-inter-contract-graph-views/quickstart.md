...
- cross_type fixture bundle at `tests/fixtures/solidity/cross_type/` (folder upload with manifest)
...
1. Load `tests/fixtures/solidity/cross_type/` as Solidity bundle.
2. Stay on **Full graph** (not Inter-contract).
3. Confirm **no** red arrow `extends OraclePeer` between `ValidatorChild` and `OraclePeer` contract boxes.
4. Click contract nodes: side panel must not offer `Edge (cross_type_call) ... extends OraclePeer` for that pair.
5. Toggle **Show cross-contract calls** off: no `extends` between bundle contracts.
6. Toggle **Show cross-contract calls** on: still **no** `extends RewardBase` between type nodes; optional function-level cross-type (e.g. `approveValidators` -> `_collateralize`) may appear.
7. **Inter-contract**: `extends` between contracts visible.

## 3. UI smoke - inter-contract

1. Switch to **Inter-contract**.
2. Between `ValidatorChild` and `OraclePeer`: one semantic link (`extends OraclePeer` or aggregated cross-type), **not** a second `solidity_import` / `bundle_import` arrow.
